...
def get_client_delivery_info(client):
    ...
    shipping_address = client.addresses.filter(
        address_type="shipping", is_primary=True
    ).first()

    # Fall back to any shipping address
    if not shipping_address:
        shipping_address = client.addresses.filter(address_type="shipping").first()
    ...
    if not shipping_address:
        shipping_address = client.addresses.filter(address_type="billing").first()
...
def deliveries_list_view(request):
    ...
    context = {
        ...
        "search_query": search,
        "breadcrumbs": (
            BreadcrumbBuilder().add_home().add_current("Deliveries").build()
        ),
    }
...
def delivery_detail_view(request, pk):
    ...
    context = {
        ...
        "breadcrumbs": (
            BreadcrumbBuilder()
            .add_home()
            .add_section("Deliveries", "deliveries:list")
            .add_current(f"Delivery: {delivery.delivery_number}")
            .build()
        ),
    }
...
def delivery_create_view(request, invoice_id=None):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request, f"Delivery {delivery.delivery_number} created successfully!"
        )
    ...
    invoices = (
        Invoice.objects.filter(
            is_active=True, status__in=["issued", "sent", "viewed", "paid", "partial"]
        )
        .select_related("client")
        .exclude(deliveries__is_active=True)
        .distinct()
    ...
    if invoice:
        auto_recipient_name, auto_delivery_location = get_client_delivery_info(
            invoice.client
        )
...
def delivery_edit_view(request, pk):
    ...
    if request.method == "POST":
        ...
        delivery.scheduled_date = (
            request.POST.get("scheduled_date") or delivery.scheduled_date
        )
        delivery.actual_delivery_date = (
            request.POST.get("actual_delivery_date") or delivery.actual_delivery_date
        )
        delivery.delivery_time = (
            request.POST.get("delivery_time") or delivery.delivery_time
        )
        delivery.delivery_method = request.POST.get(
            "delivery_method", delivery.delivery_method
        )
        delivery.delivery_location = request.POST.get(
    ...
    context = {
        ...
        "breadcrumbs": (
            ...
            .add_section("Deliveries", "deliveries:list")
            .add(
                f"Delivery: {delivery.delivery_number}",
                "deliveries:detail",
                {"pk": delivery.id},
            )
            .add_current("Edit")
    ...
    try:
        ...
        with default_storage.open(pdf_path, "rb") as pdf_file:
            ...
            response["Content-Disposition"] = (
                f'inline; filename="delivery_{delivery.delivery_number}.pdf"'
            )
