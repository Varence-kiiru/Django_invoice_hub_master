...
from invoicing_app.deliveries.models import Delivery, DeliveryLineItem, DeliveryNumberSequence
...
class DeliveryAdmin(admin.ModelAdmin):
    ...
    list_display = (
        'delivery_number', 'invoice', 'status', 'scheduled_date',
        'actual_delivery_date', 'delivery_method'
    )
    list_filter = ('status', 'delivery_method', 'scheduled_date', 'actual_delivery_date')
    search_fields = ('delivery_number', 'invoice__invoice_number', 'tracking_number')
    readonly_fields = (
        'delivery_number', 'created_at', 'updated_at', 'created_by',
        'total_items_scheduled', 'total_items_delivered'
    )
    fieldsets = (
        ('Identification', {
            'fields': ('delivery_number', 'invoice', 'created_by')
        }),
        ('Delivery Schedule', {
        ...
        ('Delivery Details', {
            'fields': ('delivery_location', 'recipient_name', 'condition', 'condition_notes')
        }),
        ('Items', {
