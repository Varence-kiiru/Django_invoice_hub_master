...
class Delivery(ActiveModel):
    ...
    scheduled_date = models.DateField(
        default=timezone.now, db_index=True, help_text="Scheduled delivery date"
    )
    actual_delivery_date = models.DateField(
        null=True, blank=True, db_index=True, help_text="Actual delivery date"
    )
    delivery_time = models.TimeField(
        null=True, blank=True, help_text="Time of delivery"
    )
    ...
    delivery_location = models.TextField(
        blank=True, null=True, help_text="Delivery address/location"
    )
    ...
    condition_notes = models.TextField(
        blank=True, null=True, help_text="Details about item condition or issues"
    )

    # ━━━ General ━━━
    notes = models.TextField(
        blank=True, null=True, help_text="Additional delivery notes"
    )
    ...
    def total_items_scheduled(self):
        ...
        return (
            self.line_items.aggregate(total=models.Sum("quantity_scheduled"))["total"]
            or 0
        )
    ...
    def total_items_delivered(self):
        ...
        return (
            self.line_items.aggregate(total=models.Sum("quantity_delivered"))["total"]
            or 0
        )
    ...
    def is_partially_delivered(self):
        ...
        if not self.line_items.exists():
            return False
        delivered_count = sum(
            1 for item in self.line_items.all() if item.quantity_delivered > 0
        )
...
class DeliveryLineItem(models.Model):
    ...
    quantity_scheduled = models.DecimalField(
        max_digits=10, decimal_places=2, help_text="Quantity scheduled for delivery"
    )
    ...
    notes = models.TextField(
        blank=True, null=True, help_text="Any notes about this item"
    )
