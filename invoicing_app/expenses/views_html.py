...
from invoicing_app.expenses.models import Expense, ExpenseCategory, Vendor, ExpenseBudget
...
def get_user_role(user):
    ...
    try:
        if user.is_superuser:
            ...
            return admin_role
        return user.invoicing_profile.role if hasattr(user, 'invoicing_profile') else None
    except:
        return None
...
def expenses_list_view(request):
    ...
    expenses_qs = expenses_qs.select_related('category', 'vendor', 'submitted_by', 'approved_by')
    ...
    if status_filter and status_filter in ['draft', 'submitted', 'approved', 'rejected', 'paid']:
    ...
    by_status = expenses_qs.values('status').annotate(count=Count('id'), total=Sum('amount')).order_by('status')
    by_category = expenses_qs.values('category__name').annotate(count=Count('id'), total=Sum('amount')).order_by('-total')
...
def expenses_detail_view(request, pk):
    ...
    context = {
        ...
        'can_approve': can_approve_expense(request.user) and expense.status == 'submitted',
        'can_reject': can_approve_expense(request.user) and expense.status == 'submitted',
...
def expenses_submit_view(request, pk):
    ...
    if expense.submitted_by != request.user and not user_has_permission(request.user, 'edit_any_expense'):
    ...
    try:
        ...
        expense.save()
        messages.success(request, f'Expense "{expense.description}" submitted for approval.')
    except Exception as e:
