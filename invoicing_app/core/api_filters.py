...
def filter_detail_api(request, filter_id):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            return JsonResponse(
                {
                    ...
                    "created_by": filter_obj.created_by.get_full_name()
                    or filter_obj.created_by.username,
                    "last_used": (
                        filter_obj.last_used.isoformat()
                        if filter_obj.last_used
                        else None
                    ),
            ...
            filter_obj.filter_criteria = data.get(
                "filter_criteria", filter_obj.filter_criteria
            )
            ...
            if filter_obj.created_by != request.user:
                return JsonResponse(
                    {"error": "Only filter owner can delete"}, status=403
                )
...
def search_suggestions_api(request):
    ...
    invoice_results = (
        ...
        .values("id", "invoice_number", "client__name")[:5]
    )
    ...
    payment_results = (
        ...
        .values("id", "invoice__invoice_number", "invoice__client__name")[:5]
    )
    ...
    suggestions["clients"] = [
        {"id": c["id"], "name": c["name"], "email": c["email"]} for c in client_results
    ]

    # Search quotations
    quote_results = (
        Quote.objects.filter(
            Q(quote_number__icontains=query) | Q(client__name__icontains=query),
            is_active=True,
        )
        .select_related("client")
        .values("id", "quote_number", "client__name")[:5]
    )
