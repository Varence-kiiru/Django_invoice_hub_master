...
class BaseModel(models.Model):
    ...
    is_active = models.BooleanField(
        default=True, db_index=True, help_text="Soft delete flag"
    )
...
class Backup(TimeStampedModel):
    ...
    file_name = models.CharField(
        max_length=255, unique=True, db_index=True, help_text="Name of backup file"
    )
    file_path = models.CharField(
        max_length=500, help_text="Full path to backup file on disk"
    )
    ...
    duration_seconds = models.PositiveIntegerField(
        null=True, blank=True, help_text="How long backup took to complete"
    )
    ...
    is_automated = models.BooleanField(
        default=False, help_text="Was this backup created automatically?"
    )
    notes = models.TextField(blank=True, null=True, help_text="Additional backup notes")
    restored_at = models.DateTimeField(
        null=True, blank=True, help_text="When this backup was last restored"
    )
...
class CompanySettings(TimeStampedModel):
    ...
    company_name = models.CharField(
        max_length=255, default="Your Company Name", help_text="Official company name"
    )
    company_website = models.URLField(
        blank=True, null=True, help_text="Company website URL"
    )
    ...
    company_email = models.EmailField(
        default="info@yourcompany.com", help_text="Primary contact email address"
    )
    company_phone = models.CharField(
        max_length=20, default="+1-234-567-8900", help_text="Primary phone number"
    )
    company_address = models.TextField(
        default="123 Business St, City, Country", help_text="Full company address"
    )
    ...
    financial_year_start = models.IntegerField(
        default=1, help_text="Month number when financial year starts (1-12)"
    )
    ...
    bank_account_name = models.CharField(
        max_length=255, blank=True, null=True, help_text="Bank account holder name"
    )
    bank_account_number = models.CharField(
        max_length=50, blank=True, null=True, help_text="Bank account number"
    )
    bank_name = models.CharField(
        max_length=255, blank=True, null=True, help_text="Bank name"
    )
    bank_branch = models.CharField(
        max_length=255, blank=True, null=True, help_text="Bank branch (optional)"
    )
...
7. This invoice is valid for payment within 30 days""",
    ...
    enable_payments = models.BooleanField(
        default=True, help_text="Enable payment tracking and recording features"
    )
    enable_reminders = models.BooleanField(
        default=True, help_text="Enable automatic payment reminders for unpaid invoices"
    )
    enable_export = models.BooleanField(
        default=True, help_text="Enable export features (PDF, Excel, Reports)"
    )
...
class EmailConfiguration(TimeStampedModel):
    ...
    smtp_port = models.IntegerField(
        default=587, help_text="SMTP port number (typically 587 for TLS, 465 for SSL)"
    )
    ...
    smtp_password_encrypted = models.TextField(
        default="", blank=True, help_text="Encrypted SMTP password (write-only)"
    )
    smtp_use_tls = models.BooleanField(
        default=True, help_text="Use STARTTLS for SMTP connection"
    )
    smtp_use_ssl = models.BooleanField(
        default=False, help_text="Use SSL for SMTP connection (typically port 465)"
    )
    ...
    enable_invoice_sent = models.BooleanField(
        default=True, help_text="Send email when invoice is marked as sent"
    )
    ...
    enable_payment_overdue = models.BooleanField(
        default=True, help_text="Send email for overdue payment reminders"
    )

    # Email Content Settings
    send_to_client_on_creation = models.BooleanField(
        default=False, help_text="Automatically send invoice to client when created"
    )
    days_before_due_reminder = models.IntegerField(
        default=3, help_text="Days before invoice is due to send reminder email"
    )
    ...
    last_tested_at = models.DateTimeField(
        null=True, blank=True, help_text="When SMTP connection was last tested"
    )
...
class FAQ(BaseModel):
    ...
    category = models.CharField(
        max_length=20, choices=CATEGORY_CHOICES, db_index=True, help_text="FAQ category"
    )
    question = models.CharField(
        max_length=500, help_text="The frequently asked question"
    )
...
class HelpArticle(BaseModel):
    ...
    title = models.CharField(max_length=300, db_index=True, help_text="Article title")
    slug = models.SlugField(
        unique=True, db_index=True, help_text="URL-friendly identifier"
    )
    category = models.CharField(
        max_length=20,
    ...
    excerpt = models.CharField(
        max_length=500, blank=True, help_text="Short preview of article content"
    )
    author = models.CharField(
        max_length=100, default="Admin", help_text="Article author"
    )
    views_count = models.IntegerField(
        default=0, help_text="Number of times this article has been viewed"
    )
    featured = models.BooleanField(
        default=False, db_index=True, help_text="Featured article on help homepage"
    )
    order = models.SmallIntegerField(
        default=0, help_text="Display order within category"
    )
...
class SupportTicket(BaseModel):
    ...
    ticket_number = models.CharField(
        max_length=20, unique=True, db_index=True, help_text="Unique ticket identifier"
    )
    name = models.CharField(max_length=200, help_text="Submitter's name")
    email = models.EmailField(db_index=True, help_text="Submitter's email address")
    subject = models.CharField(max_length=300, help_text="Support ticket subject")
    message = models.TextField(help_text="Detailed support request message")
    category = models.CharField(
        max_length=50, default="general", help_text="Support ticket category"
    )
    ...
    assigned_to = models.CharField(
        max_length=100, blank=True, help_text="Team member assigned to ticket"
    )
...
class SavedFilter(TimeStampedModel):
    ...
    description = models.TextField(
        blank=True, null=True, help_text="Description of what this filter shows"
    )
    ...
    last_used = models.DateTimeField(
        blank=True, null=True, help_text="Last time this filter was applied"
    )
    ...
    def get_user_filters(cls, user, filter_type):
        ...
        return (
            cls.objects.filter(filter_type=filter_type)
            .filter(models.Q(created_by=user) | models.Q(is_global=True))
