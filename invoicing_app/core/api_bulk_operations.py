...
def bulk_status_update(request):
    ...
    try:
        ...
        entity_type = data.get("entity_type", "").rstrip(
            "s"
        ...
        if entity_type == "invoice":
            ...
            return JsonResponse(
                {"success": False, "error": f"Unknown entity type: {entity_type}"},
                status=400,
            )
                ...
                try:
                    ...
                    if email_type == "invoice":
                        ...
                        EmailService.send_custom_email(
                            invoice.client.email, custom_subject, custom_message
                        )
            ...
            for quote_id in item_ids:
                try:
                    ...
                    if email_type == "quotation":
                        ...
                        EmailService.send_custom_email(
                            quote.client.email, custom_subject, custom_message
                        )
            ...
            for client_id in item_ids:
                try:
                    ...
                    if email_type == "custom" and custom_subject and custom_message:
                        EmailService.send_custom_email(
                            client.email, custom_subject, custom_message
                        )
                        sent_count += 1
                    else:
                        errors.append(
                            f"Client {client_id}: Unsupported email type for clients"
                        )
                except Client.DoesNotExist:
                    errors.append(f"Client {client_id} not found")
                except Exception as e:
...
def get_bulk_action_options(request):
    ...
    try:
        ...
        return JsonResponse({"success": True, "options": options[entity_type]})
