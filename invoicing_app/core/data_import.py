...
class DataImporter:
    ...
    def validate(self):
        ...
        for idx, row in enumerate(self.data):
            ...
            if errors:
                self.results["errors"].append({"row": idx + 2, "errors": errors})
...
def import_data(request):
    ...
    try:
        ...
        if not entity_type or not file:
            return JsonResponse(
                {"success": False, "error": "Missing entity_type or file"}, status=400
            )
        ...
        if not importer.parse():
            return JsonResponse(
                {"success": False, "error": importer.results["errors"][0]}, status=400
            )
    ...
    return JsonResponse(
        {
            ...
            "notes": [
                "Required fields: "
                + ", ".join(DataValidator.REQUIRED_FIELDS.get(entity_type, [])),
