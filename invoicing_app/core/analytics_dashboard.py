...
class DashboardAnalytics:
    ...
    def get_financial_metrics(self):
        ...
        avg_transaction = invoices.aggregate(Avg('total_amount'))['total_amount__avg'] or Decimal('0')
        
        # Calculate percentages
        ar_percentage = (outstanding / total_invoiced * 100) if total_invoiced > 0 else 0
    ...
    def get_invoice_metrics(self):
        ...
        return {
            ...
            'by_status': dict(invoices.values('status').annotate(count=Count('id')).values_list('status', 'count')),
            'overdue_count': invoices.filter(
                Q(status='overdue') | Q(due_date__lt=timezone.now().date(), status__in=['sent', 'issued'])
            ).count(),
            'this_month': invoices.filter(
                created_at__gte=timezone.now().replace(day=1)
            ).count(),
            'average_value': float(invoices.aggregate(Avg('total_amount'))['total_amount__avg'] or 0),
        }
    ...
    def get_payment_metrics(self):
        ...
        return {
            ...
            'by_status': dict(payments.values('status').annotate(count=Count('id')).values_list('status', 'count')),
    ...
    def get_quotation_metrics(self):
        ...
        return {
            ...
            'by_status': dict(quotations.values('status').annotate(count=Count('id')).values_list('status', 'count')),
            'conversion_rate': self._calculate_conversion_rate(),
            'average_value': float(quotations.aggregate(Avg('total_amount'))['total_amount__avg'] or 0),
        }
    ...
    def get_top_clients(self, limit=5):
        ...
        for client in clients:
            ...
            # Calculate metrics
            average_invoice = float((total_invoiced / invoice_count) if invoice_count > 0 else 0)
    ...
    def get_payment_method_breakdown(self):
        ...
        from invoicing_app.payments.models import PaymentMethod, Payment
        
        methods = PaymentMethod.objects.annotate(
            total_amount=Sum('payments__amount'),
            count=Count('payments', distinct=True)
        ).filter(count__gt=0)
...
def get_timeline_chart(request):
    ...
    return JsonResponse({
        'success': True,
        'data': analytics.get_timeline_data(min(days, 365))
    })
...
def get_aging_report(request):
    ...
    return JsonResponse({
        'success': True,
        'data': analytics.get_aging_report()
    })
...
def get_payment_method_breakdown(request):
    ...
    return JsonResponse({
        'success': True,
        'data': analytics.get_payment_method_breakdown()
    })
