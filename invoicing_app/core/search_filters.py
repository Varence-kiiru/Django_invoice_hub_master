...
class AdvancedFilterBuilder:
    ...
    def apply_payment_filters(queryset, criteria):
        ...
        if "invoice_number" in criteria and criteria["invoice_number"].strip():
            q_filter &= Q(
                invoice__invoice_number__icontains=criteria["invoice_number"].strip()
            )

        if "client_name" in criteria and criteria["client_name"].strip():
            q_filter &= Q(
                invoice__client__name__icontains=criteria["client_name"].strip()
            )
        ...
        if "valid_until_from" in criteria:
            try:
                valid_from = datetime.strptime(
                    criteria["valid_until_from"], "%Y-%m-%d"
                ).date()
...
class FilterPreset:
    ...
    @staticmethod
    def save_filter(
        name, description, filter_type, criteria, user, is_global=False, sort_by=None
    ):
    ...
    options = {
        "invoice": {
            ...
            "common_filters": [
                {
                    "name": "Unpaid",
                    "criteria": {"status": ["issued", "sent"], "amount_due_gt": 0},
                },
                {"name": "Overdue", "criteria": {"status": "overdue"}},
                {"name": "Draft", "criteria": {"status": "draft"}},
                {
                    "name": "This Month",
                    "criteria": {"from_date": timezone.now().replace(day=1)},
                },
            ],
        },
        "payment": {
            ...
            "common_filters": [
                {
                    ...
                    "criteria": {"is_active": True, "client_type": "business"},
                },
