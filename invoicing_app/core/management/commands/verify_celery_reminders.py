...
class Command(BaseCommand):
    ...
    def check_status(self, app):
        ...
        try:
            with app.connection() as conn:
                conn.connect()
            self.stdout.write(
                self.style.SUCCESS(f"✓ Broker connected: {app.conf.broker_url}")
            )
        ...
        registered_tasks = len(
            [t for t in app.tasks.keys() if not t.startswith("celery.")]
        )
        ...
        self.stdout.write(
            self.style.WARNING("  celery -A invoicing_app worker -l info")
        )
    ...
    def test_tasks(self, app):
        ...
        try:
            ...
            self.stdout.write(
                f"✓ cleanup_old_notification_logs sent (Task ID: {task3.id})"
            )
            ...
            self.stdout.write(
                self.style.WARNING(
                    "Note: Make sure Celery worker is running: celery -A invoicing_app worker -l info"
                )
            )
    ...
    def verify_config(self):
        ...
        self.stdout.write(
            "1. Start Celery worker: celery -A invoicing_app worker -l info"
        )
        self.stdout.write("2. Start Celery beat: celery -A invoicing_app beat -l info")
        self.stdout.write(
            "3. Test tasks: python manage.py verify_celery_reminders --test"
        )
        self.stdout.write(
            "4. Check status: python manage.py verify_celery_reminders --status"
        )
