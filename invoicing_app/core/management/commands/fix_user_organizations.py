...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not company_settings.company_name:
            self.stdout.write(
                self.style.ERROR('Company name not set in settings. Please configure company settings first.')
            )
            return
        ...
        if created:
            self.stdout.write(f'Created new organization: {org.name} (slug: {org.slug})')
        else:
            self.stdout.write(f'Using existing organization: {org.name} (slug: {org.slug})')
        
        # Find users without organization membership
        orphan_users = User.objects.filter(org_memberships__isnull=True)
        self.stdout.write(f'Found {orphan_users.count()} users without organization membership')
        ...
        self.stdout.write(f'Found {len(users_to_reassign)} users with different organization names')
        ...
        if dry_run:
            ...
            for user, old_org in users_to_reassign[:5]:  # Show first 5
                self.stdout.write(f'  - {user.email}: reassign from "{old_org}" to "{company_name}"')
        ...
        for user, old_org_name in users_to_reassign:
            try:
                ...
                reassigned_count += 1
                logger.info(f'Reassigned user {user.email} from "{old_org_name}" to "{org.name}"')
            except Exception as e:
                self.stdout.write(
