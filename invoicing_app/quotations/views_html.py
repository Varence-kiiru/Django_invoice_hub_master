...
from .services import QuoteNumberService, QuoteConversionService
...
def quotes_list_view(request):
    ...
    context = {
        ...
        "breadcrumbs": (
            BreadcrumbBuilder().add_home().add_current("Quotations").build()
        ),
    }
...
def quote_create_view(request):
    ...
    context = {
        ...
        "breadcrumbs": (
            BreadcrumbBuilder()
            .add_home()
            .add_section("Quotations", "quotations:list")
            .add_current("New Quotation")
            .build()
        ),
    }
...
def add_quote_line_item_view(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            tax_amount = (
                line_amount * (tax_rate.rate_percentage / 100)
                if tax_rate
                else Decimal(0)
            )
    ...
    print(f'\n{"="*70}')
    print("[add_quote_line_item_view] Building tax class rate map")
    print(f'{"="*70}')
    ...
    for tax_class in ProductTaxClass.objects.all():
        ...
        vat_rule = (
            VATRule.objects.filter(tax_class=tax_class, is_active=True)
            .order_by("-priority")
            .first()
        )
        ...
        if vat_rule:
            ...
            if vat_rule.tax_rate:
                print(
                    f"  Found TaxRate: {vat_rule.tax_rate.name} ({vat_rule.tax_rate.rate_percentage}%)"
                )
                ...
                print("  ERROR: VATRule has NO tax_rate!")
        else:
            print("  ERROR: NO VATRule found!")
    ...
    return render(
        request,
        "13_quotations/add_line_item.html",
        {
            "quote": quote,
...
def edit_quote_line_item_view(request, pk, item_id):
    ...
    if request.method == "POST":
        try:
            ...
            tax_rate_id = request.POST.get("tax_rate")
            line_item.tax_rate = (
                TaxRate.objects.get(id=tax_rate_id) if tax_rate_id else None
            )
            line_item.tax_amount = (
                line_item.line_amount * (line_item.tax_rate.rate_percentage / 100)
    ...
    print(f'\n{"="*70}')
    print("[edit_quote_line_item_view] Building tax class rate map")
    print(f'{"="*70}')
    ...
    return render(
        request,
        "13_quotations/edit_line_item.html",
        {
            "quote": quote,
            "line_item": line_item,
            "products": Product.objects.filter(is_active=True),
            "tax_rates": active_tax_rates,
            "tax_class_rate_map": json.dumps(tax_class_rate_map),
            "current_tax_rate_percentage": (
                line_item.tax_rate.rate_percentage if line_item.tax_rate else 0
            ),
...
def quote_detail_view(request, pk):
    ...
    breadcrumbs = (
        ...
        .add_home()
        .add_section("Quotations", "quotations:list")
        .add_current(f"Quote: {quote.quote_number}")
        .build()
    )
    return render(
        request,
        "13_quotations/quote_detail.html",
        {
            "quote": quote,
            "settings": settings,
...
def quote_edit_view(request, pk):
    ...
    context = {
        ...
        "breadcrumbs": (
            ...
            .add_home()
            .add_section("Quotations", "quotations:list")
            .add(f"Quote: {quote.quote_number}", "quotations:detail", {"pk": quote.id})
            .add_current("Edit")
            .build()
        ),
    }
...
def quote_delete_view(request, pk):
    ...
    return render(
        request,
        "13_quotations/quote_delete_confirm.html",
        {
            "quote": quote,
        },
    )
...
def quote_convert_view(request, pk):
    ...
    if quote.status != "accepted":
        messages.error(request, "Only accepted quotes can be converted to invoices")
        return redirect("quotations:detail", pk=pk)
    ...
    if request.method == "POST":
        ...
        try:
            ...
            invoice = QuoteConversionService.convert_quote_to_invoice(
                quote, invoice_date=invoice_date, due_date=due_date
            )
            ...
            messages.success(
                request, f"Quote converted to invoice {invoice.invoice_number}"
            )
    ...
    breadcrumbs = (
        ...
        .add_home()
        .add_section("Quotations", "quotations:list")
        .add(f"Quote: {quote.quote_number}", "quotations:detail", url_kwargs={"pk": pk})
        .add_current("Convert to Invoice")
        .build()
    )
    return render(
        request,
        "13_quotations/quote_convert.html",
        {
            "quote": quote,
            "today": today,
...
def quote_send_view(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            return JsonResponse(
                {"success": False, "message": f"Error sending quotation: {str(e)}"}
            )
    ...
    try:
        ...
        with default_storage.open(pdf_path, "rb") as pdf_file:
            ...
            response["Content-Disposition"] = (
                f'inline; filename="quotation_{quote.quote_number}.pdf"'
            )
...
def quote_issue_confirm_view(request, pk):
    ...
    breadcrumbs = (
        ...
        .add_section("Quotations", "quotations:list")
        .add(f"Quote: {quote.quote_number}", "quotations:detail", url_kwargs={"pk": pk})
        .add_current("Issue Quotation")
        .build()
    )
    context = {"quote": quote, "breadcrumbs": breadcrumbs}
...
def quote_accept_view(request, pk):
    ...
    if quote.status not in ["issued", "sent", "viewed"]:
        messages.error(
            request,
            f'Quotation with status "{quote.get_status_display()}" cannot be accepted.',
        )
        return redirect("quotations:detail", pk=pk)
    ...
    if request.method == "POST":
        try:
            ...
            messages.success(
                request, f"Quotation {quote.quote_number} marked as accepted!"
            )
            return redirect("quotations:detail", pk=pk)
        except Exception as e:
...
def quote_download_view(request, pk):
    ...
    try:
        ...
        with default_storage.open(pdf_path, "rb") as pdf_file:
            response = HttpResponse(pdf_file.read(), content_type="application/pdf")
            response["Content-Disposition"] = (
                f'attachment; filename="quotation_{quote.quote_number}.pdf"'
            )
