...
QUOTE_CONTENT_FIELDS = {
    'status', 'quote_date', 'valid_until', 'description', 'currency',
...
def update_quote_totals(sender, instance, **kwargs):
    ...
    if quote.quote_pdf:
        try:
            ...
            logger.info(f"Cleared cached PDF for quotation {quote.quote_number} due to line item change")
...
def invalidate_quote_pdf_on_content_change(sender, instance, created, update_fields, **kwargs):
    """
    ...
    if content_changed and instance.quote_pdf:
        try:
            ...
            logger.info(f"Cleared cached PDF for quotation {instance.quote_number} due to content change")
