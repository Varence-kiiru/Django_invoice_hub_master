...
def tax_rates_list(request):
    ...
    context = {
        ...
        "breadcrumbs": (
            BreadcrumbBuilder().add_home().add_current("Tax Rates").build()
        ),
    }
...
def tax_rate_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            tax_rate = form.save()
            messages.success(
                request, f'Tax rate "{tax_rate.name}" created successfully!'
            )
    ...
    context = {
        ...
        "breadcrumbs": (
            BreadcrumbBuilder()
            .add_home()
            .add_section("Tax Rates", "taxes:list")
            .add_current("New Tax Rate")
            .build()
        ),
    }
...
def tax_rate_detail(request, pk):
    ...
    related_products = (
        tax_rate.products.all().count() if hasattr(tax_rate, "products") else 0
    )
    ...
    context = {
        ...
        "breadcrumbs": (
            ...
            .add_home()
            .add_section("Tax Rates", "taxes:list")
            .add_current(f"Tax Rate #{tax_rate.id}")
            .build()
        ),
    }
...
def tax_rate_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            tax_rate = form.save()
            messages.success(
                request, f'Tax rate "{tax_rate.name}" updated successfully!'
            )
            return redirect("taxes:detail", pk=tax_rate.id)
    else:
    ...
    context = {
        ...
        "breadcrumbs": (
            ...
            .add_home()
            .add_section("Tax Rates", "taxes:list")
            .add(f"Tax Rate #{tax_rate.id}", "taxes:detail", {"pk": tax_rate.id})
            .add_current("Edit")
            .build()
        ),
    }
...
def vat_rules_list(request):
    ...
    """
    vat_rules_qs = VATRule.objects.select_related("tax_class", "tax_rate").order_by(
        "-priority", "name"
    )
    ...
    if search_query:
        vat_rules_qs = vat_rules_qs.filter(
            Q(name__icontains=search_query) | Q(tax_class__name__icontains=search_query)
        )
    ...
    context = {
        ...
        "breadcrumbs": (
            BreadcrumbBuilder().add_home().add_current("VAT Rules").build()
        ),
    }
...
def vat_rule_create(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            vat_rule = form.save()
            messages.success(
                request, f'VAT rule "{vat_rule.name}" created successfully!'
            )
            return redirect("taxes:vat-rules-list")
    else:
        form = VATRuleForm()
    ...
    context = {
        ...
        "breadcrumbs": (
            BreadcrumbBuilder()
            .add_home()
            .add_section("VAT Rules", "taxes:vat-rules-list")
            .add_current("New VAT Rule")
            .build()
        ),
    }
...
def vat_rule_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            vat_rule = form.save()
            messages.success(
                request, f'VAT rule "{vat_rule.name}" updated successfully!'
            )
            return redirect("taxes:vat-rules-list")
    else:
    ...
    context = {
        ...
        "breadcrumbs": (
            ...
            .add_home()
            .add_section("VAT Rules", "taxes:vat-rules-list")
            .add_current(f"Edit - {vat_rule.name}")
            .build()
        ),
    }
    return render(request, "taxes/vat_rule_form.html", context)
