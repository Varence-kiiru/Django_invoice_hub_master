...
class InvoiceSnapshot(models.Model):
    ...
    snapshot_date = models.DateTimeField(
        default=timezone.now, db_index=True, help_text="When this snapshot was taken"
    )
    ...
    kra_etims_receipt = models.TextField(
        blank=True, null=True, help_text="KRA eTIMS receipt/confirmation (future)"
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.pk is not None:
            raise ValueError(
                "InvoiceSnapshot is immutable; cannot update after creation"
            )
...
class AuditLog(models.Model):
    ...
    entity_id = models.BigIntegerField(help_text="ID of entity modified")
    action = models.CharField(
        max_length=20, choices=ACTION_CHOICES, db_index=True, help_text="Action taken"
    )
    old_values = models.JSONField(
        blank=True, null=True, help_text="Previous values (for updates)"
    )
    new_values = models.JSONField(
        blank=True, null=True, help_text="New values (for updates)"
    )
    ...
    timestamp = models.DateTimeField(
        default=timezone.now, db_index=True, help_text="When change occurred"
    )
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address of request (for security audit)"
    )
    user_agent = models.CharField(
        max_length=500, blank=True, null=True, help_text="Browser/app user agent"
    )
...
class LoginHistory(models.Model):
    ...
    login_time = models.DateTimeField(
        default=timezone.now, db_index=True, help_text="When user logged in"
    )
    ip_address = models.GenericIPAddressField(
        null=True, blank=True, help_text="IP address of login"
    )
    user_agent = models.CharField(
        max_length=500, blank=True, null=True, help_text="Browser/device information"
    )
    ...
    session_id = models.CharField(
        max_length=255, blank=True, null=True, help_text="Django session ID"
    )
    logout_time = models.DateTimeField(
        null=True, blank=True, help_text="When user logged out"
    )
    ...
    def set_location_from_ip(self):
        ...
        try:
            ...
            response = requests.get(
                f"https://ipapi.co/{self.ip_address}/json/", timeout=5
            )
