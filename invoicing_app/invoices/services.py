...
class TaxCalculationService:
    ...
    def calculate_invoice_totals(line_items):
        ...
        if isinstance(line_items, list):
            ...
            subtotal = line_items.aggregate(total=Sum("line_subtotal"))[
                "total"
            ] or Decimal("0.00")
            vat_total = line_items.aggregate(total=Sum("tax_amount"))[
                "total"
            ] or Decimal("0.00")
    ...
    def get_aging_buckets(days_list=None):
        ...
        for max_days in days_list:
            ...
            count = outstanding.filter(
                due_date__gte=cutoff_date, due_date__lt=prev_cutoff
            ).count()
