...
from invoicing_app.invoices.models import Invoice, InvoiceNumberSequence, InvoiceLineItem
...
def invoices_list_view(request):
    ...
    # Pagination
    invoices = paginate_queryset(request, queryset.order_by('-invoice_date'), per_page=20)
...
def add_line_item_view(request, pk):
    ...
    try:
        ...
        print(f"[add_line_item_view] Raw values: product={repr(product_id)}, qty={repr(quantity_str)}, price={repr(unit_price_str)}, tax={repr(tax_rate_str)}")
        ...
        try:
            ...
            print(f"[add_line_item_view] Tried to parse: qty={repr(quantity_str)}, price={repr(unit_price_str)}, tax={repr(tax_rate_str)}")
            return JsonResponse({
                'success': False,
                'error': 'Invalid numeric values provided'
            }, status=400)
        
        print(f"[add_line_item_view] Parsed values: qty={quantity}, price={unit_price}, tax={tax_rate}")
        ...
        try:
            ...
            available_rates = TaxRate.objects.all().values_list('rate_percentage', flat=True)
            return JsonResponse({
                'success': False,
        ...
        if existing_item:
            ...
            existing_item.tax_amount = existing_item.line_amount * (existing_item.tax_rate.rate_percentage / 100)
            existing_item.line_total = existing_item.line_amount + existing_item.tax_amount
            existing_item.save()
            line_item = existing_item
            print(f"[add_line_item_view] Consolidated: Updated existing item {line_item.id} quantity to {line_item.quantity}")
        else:
...
def edit_line_item_view(request, pk, line_item_id):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({
                'success': False,
                'error': f'Tax rate {tax_rate}% not found'
            }, status=400)
        ...
        return JsonResponse({
            'success': False,
            'error': f'Error updating line item: {str(e)}'
        }, status=500)
...
def remove_line_item_view(request, pk, line_item_id):
    ...
    try:
        ...
        return JsonResponse({
            'success': False,
            'error': f'Error removing line item: {str(e)}'
        }, status=500)
...
def invoices_display_pdf_view(request, pk):
    ...
    try:
        ...
        context = {
            ...
            # Bank Details
            'bank_account_name': company_settings.bank_account_name if company_settings else '',
            'bank_account_number': company_settings.bank_account_number if company_settings else '',
            ...
            'bank_swift_code': company_settings.bank_swift_code if company_settings else '',
            'bank_iban': company_settings.bank_iban if company_settings else '',
            # M-Pesa Details
            'mpesa_paybill_number': company_settings.mpesa_paybill_number if company_settings else '',
            'mpesa_account_name': company_settings.mpesa_account_name if company_settings else '',
...
def invoices_view_pdf_view(request):
    ...
    try:
        ...
        # Build queryset
        invoices = Invoice.objects.filter(is_active=True).select_related('client', 'created_by')
...
def invoices_send_view(request, pk):
    ...
    return render(request, '6_invoices/invoice_send_email.html', context)


@login_required
def invoice_issue_confirm_view(request, pk):
    """Confirm and issue invoice."""
    from django.contrib import messages
    invoice = get_object_or_404(Invoice, pk=pk, status='draft')
    if request.method == 'POST':
...
def invoice_clone_view(request, pk):
    ...
    if request.method == 'POST':
        try:
            ...
            new_invoice_number = InvoiceNumberService.generate_next_number(prefix=prefix)
            ...
            cloned_invoice.subtotal_amount = line_items.aggregate(Sum('line_amount'))['line_amount__sum'] or 0
            cloned_invoice.vat_amount = line_items.aggregate(Sum('tax_amount'))['tax_amount__sum'] or 0
            cloned_invoice.total_amount = line_items.aggregate(Sum('line_total'))['line_total__sum'] or 0
            cloned_invoice.amount_due = cloned_invoice.total_amount
            cloned_invoice.save()
            
            messages.success(request, f'Invoice {new_invoice_number} created from {original_invoice.invoice_number}!')
