    ...
    try:
        ...
        for invoice in overdue_invoices:
            ...
            AuditLog.objects.create(
                ...
                action='updated',
                old_values={'status': 'issued' if invoice.status == 'issued' else 'sent'},
            ...
            try:
                if invoice.client.email:
                    ...
                    if success:
                        ...
                        invoice.save(update_fields=['first_reminder_sent_at', 'updated_at'])
                ...
                logger.error(f"Error sending overdue reminder for {invoice.invoice_number}: {str(e)}")
...
def send_invoice_email_task(invoice_id):
    ...
    try:
        ...
        if success:
            ...
            logger.info(f"Invoice {invoice.invoice_number} sent to {invoice.client.email}")
            return {'success': True, 'message': f'Invoice sent to {invoice.client.email}'}
        else:
...
def generate_invoice_pdf_task(invoice_id, save_to_storage=True):
    ...
    try:
        ...
        try:
            ...
            pdf_path = pdf_service.generate_invoice_pdf(invoice_id, save=save_to_storage)
        except (ImportError, AttributeError):
    ...
    try:
        ...
        if success:
            ...
            if reminder_number in field_map:
                update_data = {field_map[reminder_number]: timezone.now(), 'updated_at': timezone.now()}
            ...
            logger.info(f"Sent payment reminder {reminder_number} for invoice {invoice.invoice_number}")
