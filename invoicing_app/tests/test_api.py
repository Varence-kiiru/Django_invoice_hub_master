...
class ApiTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='apiuser', email='api@example.com', password='secret123')
        self.client = APIClient()

    def obtain_token(self):
        resp = self.client.post('/api/v1/token/', {'username': 'apiuser', 'password': 'secret123'}, format='json')
