...
class PDFService:
    ...
    def _generate_invoice_qr_code(invoice):
        ...
        try:
            ...
            logger.info(
                f"Generated comprehensive QR code for invoice {invoice.invoice_number}"
            )
            return data_uri
        except Exception as e:
    ...
    def _generate_quote_qr_code(quote):
        ...
        try:
            ...
            data_uri = f"data:image/png;base64,{img_data}"
            logger.info(
                f"Generated comprehensive QR code for quote {quote.quote_number}"
            )
            return data_uri
        except Exception as e:
            logger.warning(f"Error generating QR code: {str(e)}")
    ...
    def generate_invoice_pdf(invoice_id, save=True):
        ...
        try:
            ...
            if (
                save
                and invoice.invoice_pdf
                and default_storage.exists(invoice.invoice_pdf.name)
            ):
            ...
            context = {
                ...
                "company_logo": (
                    logo_data_uri if logo_data_uri else None
                ),  # Use data URI for PDF
                "invoice_qr_code": (
                    qr_code_data_uri if qr_code_data_uri else None
                ),  # Add QR code
            ...
            html_string = render_to_string(
                "invoicing_app/invoices/invoice_pdf.html", context
            )
    ...
    def generate_quote_pdf(quote_id, save=True):
        ...
        try:
            ...
            if (
                save
                and quote.quote_pdf
                and default_storage.exists(quote.quote_pdf.name)
            ):
            ...
            # Render HTML template
            html_string = render_to_string("13_quotations/quote_pdf.html", context)
    ...
    def generate_invoice_batch_pdf(invoice_ids, save=True):
        ...
        try:
            ...
            if save:
                ...
                path = default_storage.save(filename, ContentFile(pdf_content))
                logger.info(
                    f"Saved batch PDF with {len(invoice_ids)} invoices to {path}"
                )
                return path
            else:
                return BytesIO(pdf_content)
    ...
    def generate_payment_receipt_pdf(payment_id, save=True):
        ...
        try:
            ...
            payment = Payment.objects.select_related(
                "invoice", "invoice__client", "payment_method", "recorded_by"
            ).get(id=payment_id)

            # Check if PDF already exists (only when saving)
            if (
                save
                and payment.receipt_pdf
                and default_storage.exists(payment.receipt_pdf.name)
            ):
                logger.info(
                    f"PDF receipt already exists for payment {payment_id}: {payment.receipt_pdf.name}"
                )
            ...
            previous_payments = (
                payment.invoice.payments.exclude(id=payment.id).aggregate(
                    total=Sum("amount")
                )["total"]
                or 0
            )
            ...
            # Render HTML template
            html_string = render_to_string(
                "7_payments/payment_receipt_pdf.html", context
            )
    ...
    def generate_report_pdf(report_type, context, template_name, filename_prefix):
        ...
        try:
            ...
            logger.info(
                f"Generated {report_type} report PDF ({len(pdf_content)} bytes)"
            )
            return pdf_content
    ...
    def generate_delivery_pdf(delivery_id, save=True):
        ...
        try:
            ...
            if (
                save
                and hasattr(delivery, "delivery_pdf")
                and delivery.delivery_pdf
                and default_storage.exists(delivery.delivery_pdf.name)
            ):
            ...
            # Render HTML template
            html_string = render_to_string("14_deliveries/delivery_pdf.html", context)
            ...
            if save:
                ...
                logger.info(
                    f"Saved PDF for delivery {delivery.delivery_number} to {path}"
                )
                return path
            else:
                # Return BytesIO object
