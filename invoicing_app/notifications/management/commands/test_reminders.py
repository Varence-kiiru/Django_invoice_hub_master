...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("🚀 Testing Email Reminder System"))
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"❌ Error testing reminders: {str(e)}"))
    ...
    def _print_result(self, result, label):
        ...
        if status == "success":
            ...
            self.stdout.write(self.style.WARNING(f"  ⚠️  {label}: Disabled in settings"))
        else:
            self.stdout.write(self.style.ERROR(f"  ❌ {label}: {error or status}"))
