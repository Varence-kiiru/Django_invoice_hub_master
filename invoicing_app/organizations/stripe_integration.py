...
class SubscriptionManager:
    ...
    def ensure_stripe_products():
        ...
        try:
            for plan_key, plan_config in SubscriptionManager.PLAN_PRICING.items():
                ...
                logger.info(
                    f"Created Stripe product {product.id} and price {price.id} for plan {plan_key}"
                )
    ...
    def create_customer(organization):
        ...
        try:
            ...
            logger.info(
                f"Created Stripe customer {customer.id} for org {organization.slug}"
            )
            return customer
    ...
    def create_subscription(organization, plan, payment_method_id=None):
        ...
        try:
            ...
            subscription = Subscription.objects.create(
                ...
                plan=plan,
                status=(
                    "active" if stripe_subscription.status == "active" else "trialing"
                ),
            ...
            logger.info(
                f"Created subscription {stripe_subscription.id} for org {organization.slug}"
            )
            return subscription
    ...
    def cancel_subscription(organization, reason="customer_request"):
        ...
        try:
            ...
            organization.save(
                update_fields=["status", "plan", "stripe_subscription_id"]
            )

            logger.info(
                f"Cancelled subscription for org {organization.slug} - reason: {reason}"
            )
            return True
    ...
    def retry_failed_payment(organization):
        ...
        try:
            ...
            stripe.Subscription.modify(stripe_sub.id, off_session=True)
    ...
    def _handle_subscription_updated(stripe_subscription):
        ...
        try:
            ...
            logger.warning(
                f"Org not found for subscription {stripe_subscription['id']}"
            )
...
class BillingService:
    ...
    def generate_invoice(organization, amount, description):
        ...
        invoice_number = Invoice.objects.filter(organization=organization).count() + 1
    ...
    def get_churn_rate(days=30):
        ...
        cancelled = Subscription.objects.filter(
            status="cancelled", updated_at__gte=start_date
        ).count()
