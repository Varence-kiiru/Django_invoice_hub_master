...
from .stripe_integration import SubscriptionManager, BillingService
...
def get_user_organization(user):
    ...
    try:
        ...
        primary_memberships = OrganizationMember.objects.filter(user=user, is_primary=True)
        ...
        if primary_memberships.count() > 1:
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.warning(f"User {user.email} has {primary_memberships.count()} primary organization memberships. Using first one.")
...
def billing_dashboard_view(request):
    ...
    recent_invoices = Invoice.objects.filter(organization=organization).order_by('-created_at')[:10]
...
def plan_upgrade_view(request):
    ...
    if request.method == 'POST':
        ...
        if new_plan != 'free':
            ...
            if not organization.stripe_customer_id:
                try:
                    ...
                    logger.error(f"Failed to create Stripe customer for {organization.slug}: {str(e)}")
            ...
            try:
                ...
                logger.error(f"Failed to check payment methods for {organization.slug}: {str(e)}")
                messages.error(request, "Failed to verify payment method. Please try again.")
                return redirect('organizations:upgrade')
        ...
        try:
            ...
            if subscription:
                messages.success(request, f"Successfully upgraded to {new_plan.title()} plan!")
                logger.info(f"Organization {organization.slug} upgraded to {new_plan}")
                return redirect('organizations:billing_dashboard')
            else:
                messages.error(request, "Failed to upgrade subscription. Please try again.")
                return redirect('organizations:upgrade')
...
def get_current_usage(organization, subscription):
    ...
    return {
        ...
        'users': team_members,
        'users_limit': plan_limits['max_users'] if plan_limits['max_users'] else 'Unlimited',
