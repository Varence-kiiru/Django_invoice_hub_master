...
class InvoiceCreatePermission(permissions.BasePermission):
    ...
    message = (
        "You have reached your invoice quota for this month. Please upgrade your plan."
    )
    ...
    def has_permission(self, request, view):
        ...
        if (
            ...
            and "invoice" in view.__class__.__name__.lower()
        ):
            quota_check = PlanEnforcer.check_invoice_quota(request.user)

            if (
                not quota_check["allowed"]
                and quota_check.get("reason") == "quota_exceeded"
            ):
...
class TeamMemberPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        if (
            request.method in ["POST", "PUT", "PATCH"]
            and "member" in view.__class__.__name__.lower()
        ):
...
class FeatureAccessPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        """
        # Get user's plan
        organization, plan, subscription = PlanEnforcer.get_organization_plan(
            request.user
        )
