        ...
        if form.is_valid():
            try:
                ...
                if company_settings.company_name != DEFAULT_COMPANY_NAME:
                    ...
                    logger.info(f"Using existing company name from settings: '{actual_company_name}' (ignoring form: '{company_name}')")
                else:
                    # Still default, save user's input from form
                    actual_company_name = company_name
                    company_settings.company_name = company_name
                    logger.info(f"Saved company name from signup form to settings: '{company_name}'")
                ...
                if company_website and company_website.strip():
                    ...
                    logger.info(f"Saved company website from signup form: '{company_website}'")
                ...
                user = User.objects.create_user(
                    username=email.split('@')[0] + '_' + timezone.now().strftime('%Y%m%d%H%M%S'),
                    email=email,
                ...
                if User.objects.count() == 1:  # This is the first user
                    ...
                    user.is_staff = True
                    user.save()
                    logger.info(f"First user registered - granted superuser status: {email}")
                ...
                logger.info(f"DEBUG: Creating organization with slug={slug}, email={email}")
                ...
                logger.info(f"DEBUG: Organization created: {organization.id}, {organization.slug}")
                ...
                # Log the signup
                logger.info(f"New signup: {email} created organization {organization.slug}")
                ...
                messages.success(request, f"Welcome {first_name}! We've sent a verification email to {email}. Let's set up your company profile.")
                ...
                messages.error(request, f"An error occurred during signup. Please try again. Error: {str(e)}")
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                company_settings.company_phone = form.cleaned_data.get('company_phone', '')
                company_settings.company_email = form.cleaned_data.get('company_email', company_settings.company_email)
                company_settings.company_website = form.cleaned_data.get('company_website', '')
                company_settings.company_address = form.cleaned_data.get('company_address', '')
                ...
                company_settings.company_address = ', '.join(filter(None, address_parts)) if address_parts else company_settings.company_address
                ...
                organization.admin_email = form.cleaned_data.get('company_email', organization.admin_email)
                organization.save(update_fields=['name', 'admin_email'])
                
                messages.success(request, "Company profile updated successfully! Your information is now stored in settings.")
        ...
        initial_data = {
            ...
            'company_address': company_settings.company_address.split(',')[0].strip() if company_settings.company_address else '',
            'company_tax_id': company_settings.tax_id,
        }
        form = CompanySetupForm(initial=initial_data)
    
    return render(request, 'auth/company_setup.html', {
        'form': form,
        'organization': organization,
    ...
    if request.user.is_authenticated:
        ...
        try:
            ...
            if token == expected_token:
                ...
                user.save()
                messages.success(request, "✓ Your email has been verified! You can now use all features.")
...
def password_reset_view(request):
    ...
    if request.method == 'POST':
        ...
        if not email:
            ...
            return render(request, 'auth/password_reset.html', {'page_title': 'Reset Your Password'})
        ...
        try:
            ...
            reset_token = hashlib.sha256(
                f"{user.id}:{user.email}:{settings.SECRET_KEY}:{timezone.now().strftime('%Y-%m-%d')}".encode()
            ).hexdigest()
