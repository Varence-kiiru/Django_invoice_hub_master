...
from invoicing_app.payments.models import Payment, PaymentMethod, PaymentReconciliation
...
def confirm_payment_view(request, pk):
    ...
    messages.success(request, f'Payment {payment.transaction_reference or payment.id} confirmed!')
...
def reverse_payment_view(request, pk):
    ...
    invoice = payment.invoice
    if invoice.amount_due > 0:
        invoice.status = 'issued' if invoice.amount_due == invoice.total_amount else 'partial'
        invoice.paid_date = None
        invoice.save()
    
    messages.success(request, f'Payment {payment.transaction_reference or payment.id} reversed!')
    return redirect('payments:detail', pk=pk)
...
def payments_list_view(request):
    ...
    if not settings.enable_payments:
        messages.error(request, 'Payment tracking is currently disabled by your administrator')
    ...
    # Pagination
    payments = paginate_queryset(request, queryset.order_by('-payment_date'), per_page=20)
...
def payments_create_view(request):
    ...
    if invoice_id_param:
        try:
            pre_selected_invoice = Invoice.objects.get(id=invoice_id_param, is_active=True)
    ...
    if request.method == 'POST':
        try:
            ...
            if amount > invoice.amount_due:
                messages.error(request, f'Payment amount cannot exceed invoice balance of {invoice.amount_due}')
                return redirect('payments:create')
            
            # Get payment prefix from settings
            settings = CompanySettings.get_settings()
            receipt_number = PaymentReceiptNumberService.generate_next_number(prefix=settings.payment_prefix)
            ...
            messages.success(request, f'Payment of {payment.amount} recorded and reconciled!')
...
def payments_detail_view(request, pk):
    ...
    total_payments = payment.invoice.payments.aggregate(total=Sum('amount'))['total'] or 0
...
def payment_reconciliation_view(request):
    ...
    payments = Payment.objects.filter(
        status='pending'
    ).select_related('invoice__client', 'payment_method')
...
def payment_matching_view(request, pk):
    ...
    payment = get_object_or_404(Payment, pk=pk)
    context = {'page_title': 'Match Payment', 'payment': payment}
...
def payment_receipt_download_view(request, pk):
    ...
    try:
        ...
        logger.info(f"Served payment receipt PDF {payment.receipt_number or payment.id} for download")
        return response
...
def payment_methods_list_view(request):
    ...
    from invoicing_app.core.views_html import _get_user_role, role_required
...
def payment_method_create_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            payment_method.save()
            messages.success(request, f'Payment method "{payment_method.name}" created!')
...
def payment_method_edit_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            payment_method.updated_by = request.user
            payment_method.save()
            messages.success(request, f'Payment method "{payment_method.name}" updated!')
            return redirect('payments:methods-list')
        else:
            for error in form.errors.values():
                messages.error(request, error)
...
def payment_method_delete_view(request, pk):
    ...
    if Payment.objects.filter(payment_method=payment_method).exists():
        messages.error(request, f'Cannot delete "{name}" - it is in use by payments. Mark as inactive instead.')
        return redirect('payments:methods-list')
