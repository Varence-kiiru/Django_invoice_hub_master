...
class ClientAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Info', {
            'fields': ('name', 'uuid', 'client_type', 'email', 'phone')
        }),
        ('Tax & Registration', {
            'fields': ('tax_id', 'business_registration_number')
        }),
        ('Billing', {
            'fields': ('currency', 'default_tax_rate', 'payment_terms_days', 'credit_limit')
        }),
        ('Audit', {
            'fields': ('is_active', 'created_by', 'created_at', 'updated_at', 'notes'),
            'classes': ('collapse',)
        }),
    )
